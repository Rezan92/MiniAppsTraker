...
export const DatePicker = ({ value, onChange, placeholder = 'Select date', className = '' }) => {
  ...
  const updateCoords = () => {
    if (buttonRef.current) {
      ...
      const dropdownWidth = dropdownRef.current ? dropdownRef.current.offsetWidth : 300;
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                {!showMonthYearPicker && (
                  <div className="flex items-center gap-1">
                    <button type="button" onClick={prevMonth} className="p-1 rounded-full hover:bg-gray-100 transition-colors cursor-pointer" aria-label="Previous month">
                      <span className="material-symbols-outlined text-gray-500 text-xl">chevron_left</span>
                    </button>
                    <button type="button" onClick={nextMonth} className="p-1 rounded-full hover:bg-gray-100 transition-colors cursor-pointer" aria-label="Next month">
              ...
              {showMonthYearPicker ? (
                <div>
                  ...
                  <div className="flex items-center justify-between mb-3 px-1">
                    <button type="button" onClick={() => handleYearChange(year - 1)} className="p-1 rounded-full hover:bg-gray-100 transition-colors cursor-pointer" aria-label="Previous year">
                      <span className="material-symbols-outlined text-gray-500 text-xl">chevron_left</span>
                    </button>
                    <span className="text-sm font-bold text-gray-900">{year}</span>
                    <button type="button" onClick={() => handleYearChange(year + 1)} className="p-1 rounded-full hover:bg-gray-100 transition-colors cursor-pointer" aria-label="Next year">
                      <span className="material-symbols-outlined text-gray-500 text-xl">chevron_right</span>
                    </button>
                  </div>
                  {/* Month Grid */}
                ...
                <>
                  ...
                  <div className="grid grid-cols-7 gap-1 mb-1">
                    {DAYS.map((d, i) => (
                      <div key={i} className="h-9 w-9 mx-auto flex items-center justify-center text-xs font-medium text-gray-400">{d}</div>
                    ))}
                  </div>
                  {/* Day Grid */}
