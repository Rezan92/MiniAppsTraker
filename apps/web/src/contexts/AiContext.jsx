...
export const AiContextProvider = ({ children }) => {
  ...
  const sendMessage = useCallback(async (content, attachment = null) => {
    ...
    try {
      const apiMessages = updatedHistory.map((m, idx) => {
        const isLatest = idx === updatedHistory.length - 1;
        return {
          role: m.role,
          content: m.content || '',
          attachment: m.attachment ? {
            mimeType: m.attachment.mimeType,
            // Optimization: Only send heavy base64 data for the latest message being sent
            data: isLatest ? m.attachment.data : null,
