...
router.post('/chat', async (req, res, next) => {
  try {
    ...
    // Format chat history for @google/genai (supporting multimodal attachments with historical pruning)
    const contents = messages.map((m, index) => {
      const parts = [];
      const isLatestMessage = index === messages.length - 1;

      if (m.attachment) {
        // Only include inlineData for the current active incoming turn if raw data exists
        if (isLatestMessage && m.attachment.data) {
          ...
          // Historical attachment pruning: save tokens by substituting a lightweight reference
          parts.push({
            text: `[Attached image: ${m.attachment.name || 'photo'} (previously analyzed)]`
          });
        }
      }
    ...
    // Multi-turn tool execution loop (up to 8 turns)
    let currentTurn = 0;
    const maxTurns = 8;
